...
- Dashboard Admin tenta `GET /api/snapshots?limit=30` e `GET /api/journey-entities?limit=50` junto de eventos, status e tabelas; `data-admin-backend-snapshots`/`data-admin-backend-snapshot` e `data-admin-backend-entities`/`data-admin-backend-entity` identificam os itens recentes.
...
| `BFBackendApi` | Ponte progressiva para API local Node/SQLite: sessao de backend, usuarios, status tecnico do banco, importacao guiada, gravacao/listagem de eventos, gravacao/listagem de snapshots, leitura de entidades relacionais e fallback estatico. |
