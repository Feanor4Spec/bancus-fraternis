...
| P0 | Adapter produtivo piloto | Implementado e aprovado localmente; falta smoke em PostgreSQL externo de homologacao. | `js/backend/providers/*`, `server.js`, `tools/validate-database-provider.mjs`, `docs/POSTGRESQL_PILOT.md`. | Gate injetado 38/38; `BFBackendApi` e `/api/*` preservam semantica; provider sem credencial, TLS ou migration falha explicitamente. |
