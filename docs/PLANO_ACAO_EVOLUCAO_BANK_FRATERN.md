...
| P0 | Resultado como decisao | Transformar o resumo financeiro em recomendacao final com riscos, premissas, comparacao e CTA para proposta. | `js/app.js`, `js/simulator-cart.js`, `js/proposal-summary.js`. | Resultado deixa claro qual grupo/cenario seguir, o que revisar e quando gerar proposta. |
| P0 | Filtros e fila dedicada no Admin | Separar leads, simulacoes e propostas em uma fila propria com filtros por tipo, status, prioridade e dono, mantendo a edicao inline atual. | `assets/js/admin-users.js`, `assets/css/platform.css`, `tools/validate-local-database.mjs`. | Admin encontra rapidamente registros dedicados e consegue priorizar o proximo atendimento sem navegar pela auditoria completa. |
